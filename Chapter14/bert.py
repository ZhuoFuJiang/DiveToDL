...
class MaskLM(nn.Module):
    ...
    def forward(self, X, pred_positions):
        # num_pred_positions表示需预测的掩码数
        num_pred_positions = pred_positions.shape[1]
        # pred_positions表示掩码位置
        ...
        # masked_X的形状为：(batch_size 预测掩码数 隐藏状态数)
        masked_X = masked_X.reshape((batch_size, num_pred_positions, -1))
        # mlm_Y_hat的形状为：(batch_size, 预测掩码数, 词表数)
...
# encoded_X的形状为：(batch_size time_steps * num_hiddens)
...
class BERTModel(nn.Module):
    ...
    def forward(self, tokens, sgements, valid_lens=None, pred_positions=None):
